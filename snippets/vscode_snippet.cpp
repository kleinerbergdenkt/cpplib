...
namespace gengar094
{
	...
	template<class T,class U>void Print(const std::pair<T,U>&a){Print(a.first);std::cout<<' ';Print(a.second);}
	template<class T>void Print(const T&a){std::cout<<a;}
	void out(){std::cout<<std::endl;}
	template<class T>void out(const T&t){Print(t);std::cout<<std::endl;}
	template<class Car,class...Cdr>void out(Car&car,Cdr&...cdr){Print(car);std::cout<<' ';out(std::forward<Cdr>(cdr)...);}
